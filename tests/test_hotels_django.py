"""
Comprehensive Django test suite for hotels app - MASTER FIX validation
Tests cover: mapping, pricing, validation firewall, queryset optimization
Using Django's unittest framework (no pytest required)
"""
...
class TestPropertyMapping(TestCase):
	...
	def test_property_creation_with_all_fields(self):
		...
		self.assertEqual(property_obj.name, 'Test Hotel')
		self.assertEqual(property_obj.rating, Decimal('4.5'))
	...
	def test_property_images_ordering(self):
		...
		self.assertTrue(images[0].is_featured)
		self.assertEqual(images[0], img2)


class TestPricingDeterministic(TestCase):
	...
	def test_base_price_calculation(self):
		...
		self.assertEqual(property_obj.base_price, Decimal('3000.00'))
	...
	def test_discount_price_logic(self):
		...
		self.assertEqual(property_obj.discount_price, Decimal('4500.00'))


class TestValidationFirewall(TestCase):
	...
	def test_negative_price_rejected(self):
		...
		with self.assertRaisesMessage(ValidationError, 'Price cannot be negative'):
...
class TestQuerysetOptimization(TestCase):
	"""Test queryset optimization with select_related/prefetch_related"""

	def test_hotel_list_uses_optimized_queries(self):
		"""Verify hotel_list view uses optimized queries and returns 200"""
...
class TestCategorySeeding(TestCase):
	...
	def test_categories_can_be_created(self):
		...
		self.assertEqual(category.slug, 'beach-vacations')
