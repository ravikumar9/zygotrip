...
class FilterService:
    ...
    def _get_payment_mode_filters(queryset):
        """Payment modes (would require booking data)"""
        # For now, return hardcoded list with placeholder counts
        return [
            {'mode': 'Pay at Property', 'count': len(queryset)},
            {'mode': 'Card Payment', 'count': len(queryset)},
            {'mode': 'UPI/Wallet', 'count': len(queryset) // 2},
        ]
