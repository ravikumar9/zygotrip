...
def get_filter_counts(queryset):
    ...
    # Compute amenity counts — one query via GROUP BY
    amenity_counts = dict(
        base_qs
        .values('amenities__name')
        .annotate(count=Count('id', distinct=True))
        .filter(amenities__name__isnull=False)
    ...
    counts = {
        ...
        # Use template-friendly keys (no + or . characters)
        'user_ratings': {
            'rating_4_5plus': base_qs.filter(avg_rating__gte=4.5).count(),
            'rating_4_0plus': base_qs.filter(avg_rating__gte=4.0).count(),
            'rating_3_5plus': base_qs.filter(avg_rating__gte=3.5).count(),
        },
...
def apply_sorting(queryset, sort_param):
    ...
    elif sort_param in ('rating', 'rating_high'):
